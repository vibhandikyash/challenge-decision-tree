...
const BidCapService = {
    getConditionExpression({ rpcAlpha, rpcBeta, ebRpc, net, nonSocialClicks, nonSocialClicksCutOff, socialClicks, socialClicksCutOff, currentBidCap, factor
    }) {
        const avgAlphaBeta = (rpcAlpha + rpcBeta) / 2;
        const maxValue = Math.max(ebRpc, avgAlphaBeta);
        const socialClickCondition = socialClicks > socialClicksCutOff && nonSocialClicks > nonSocialClicksCutOff;
